...
- `packages/invoice-repository-dynamodb` is the scaffolded future durable DynamoDB adapter. It is
  separate from the storage-neutral repository contracts and the in-memory adapter; its factory
  intentionally throws, and no DynamoDB persistence behavior or AWS SDK dependency exists yet.
